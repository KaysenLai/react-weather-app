export const STORE_CURRENT_WEATHER = 'storeCurrentWeather';
export const STORE_HOURLY_WEATHER = 'storeHourlyWeather';
export const STORE_DAILY_WEATHER = 'storeDailyWeather';
export const SET_SAVEBOX_WEATHER = 'setSaveboxWeather';

export const STORE_SEARCH_CITY = 'storeSearchCity';
export const STORE_CLICKED_CITY = 'storeClickedCity';
export const CLEAT_SEARCH_CITY = 'clearSearchCity';

export const STORE_ONE_IMG = 'storeOneImg';
export const STORE_SEARCH_IMG = 'storeSearchImg';
export const SET_MAIN_IMG = 'setMainImg';
export const SET_SAVEBOX_IMG = 'setSavaBoxImg';

export const STORE_THEME_COLOR = 'storeThemeColor';
